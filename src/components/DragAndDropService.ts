import { Ref, ref, onMounted, onBeforeUnmount, onBeforeUpdate, nextTick } from '@vue/composition-api';

type DragEl = HTMLElement | undefined
type DropEl = HTMLElement | undefined
type DragCb = (e: DragEvent) => void
  ...
  constructor(props: DragServiceProps) {
    ...
    onMounted(async () => {
      ...
      if (this.dragRefList.value.length <= 0) {
        throw new Error('please bind the dragRefList, by ref="el=>dragListRef[i]=el"')
      }
      if (this.dropRefList.value.length <= 0) {
        throw new Error('please bind the dropRefList, by ref="el=>dragListRef[i]=el"')
      }
      for (let key in this.dragRefList.value) {
        ...
        dragRef.addEventListener('dragstart', (e: DragEvent) => this.start.bind(this)(e, parseInt(key)))
      ...
      for (let key in this.dropRefList.value) {
        ...
        dropRef.addEventListener('drop', (e: DragEvent) => this.drop.bind(this)(e, parseInt(key)))
      }
    })
    ...
    onBeforeUnmount(() => {
      for (let key in this.dragRefList.value) {
        ...
        dragRef.removeEventListener('dragstart', (e: DragEvent) => this.start.bind(this)(e, parseInt(key)))
      ...
      for (let key in this.dropRefList.value) {
        ...
        dropRef.removeEventListener('drop', (e: DragEvent) => this.drop.bind(this)(e, parseInt(key)))
      }
    })
  }
